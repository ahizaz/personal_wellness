...
class SignInController extends GetxController {
 ...
 ///signIn with apple 
  Future<void> signInWithApple() async {
    ...
    if (kIsWeb || !(Platform.isIOS || Platform.isMacOS)) {
      EasyLoading.showInfo(
        'Apple Sign-In is available only on iOS / macOS devices',
      );
      return;
    }
    ...
    try {
      ...
      final tokenPayload = _decodeJwtPayload(credential.identityToken);
      final emailFromToken = tokenPayload["email"]?.toString().trim();
      final userIdFromToken = tokenPayload["sub"]?.toString().trim();
      final credentialEmail = credential.email?.trim();
      final email = (credentialEmail != null && credentialEmail.isNotEmpty)
          ? credentialEmail
          : ((emailFromToken != null && emailFromToken.isNotEmpty)
                ? emailFromToken
                : null);
      final userId =
          (credential.userIdentifier != null &&
              credential.userIdentifier!.trim().isNotEmpty)
          ? credential.userIdentifier!.trim()
          : ((userIdFromToken != null && userIdFromToken.isNotEmpty)
                ? userIdFromToken
                : null);
      ...
      final fallbackName = (email != null && email.contains("@"))
          ? email.split("@").first
          : "Apple User";
      final displayName = providedName.isNotEmpty ? providedName : fallbackName;
  ...
  Future<void> signInWithGoogle() async {
    ...
    if (kIsWeb || !Platform.isAndroid) {
      EasyLoading.showInfo(
        'Google Sign-In is available only on Android devices',
      );
      return;
    }
