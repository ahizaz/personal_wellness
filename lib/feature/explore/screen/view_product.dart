...
class ViewProduct extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // Apply payload from both Explore and SkinCondition->Recommended
    final dynamic args = Get.arguments;
    if (args is Map<String, dynamic>) {
      controller.applyProductArg(args);
    } else if (args is String) {
      controller.productDataview['productName'] = args; // backward compatibility
    }
    ...
    return Scaffold(
       ...
       body:SafeArea(child: SingleChildScrollView(
        ...
        child: Column(
              ...
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                  const ProductHeader(title: 'Product',),
                  SizedBox(height: 9.h),
                 Stack(
  alignment: Alignment.center,
  children: [
    ...
    Obx(() {
      ...
      final index = controller.currentIndex.value.clamp(0, images.isEmpty ? 0 : images.length - 1);
      final String img = images.isNotEmpty ? images[index] : "";
      ...
      if (img.startsWith('http') || img.startsWith('https')) {
        ...
        provider = const AssetImage("assets/images/produt_details_1.png");
      }
    ...
    // Right arrow - only show if not on last image
    Obx(() {
      final maxIndex = controller.imageUrls.isEmpty ? 0 : controller.imageUrls.length - 1;
      return controller.currentIndex.value < maxIndex
        ? Positioned(
            right: 16.w,
            child: InkWell(
              onTap: controller.nextImage,
