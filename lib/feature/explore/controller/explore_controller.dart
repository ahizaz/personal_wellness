...
class ExploreController extends GetxController {
  ...
  // Keep original type for products
  final RxList<Map<String, String>> products = <Map<String, String>>[].obs;
        ...
        if (data["success"] == true && data["data"]["result"] != null) {
          ...
          for (var item in data["data"]["result"]) {
            ...
            if (item["image"] is List && (item["image"] as List).isNotEmpty) {
              firstImage = "${Urls.imageurl}${item["image"][0]}";
            }
        ...
        if (data["success"] == true && data["data"]["result"] != null) {
          ...
          for (var item in data["data"]["result"]) {
            final List<String> images = (item["image"] is List)
                ? (item["image"] as List).map((e) => "${Urls.imageurl}$e").cast<String>().toList()
                : <String>[];
            ...
            recommendedProducts.add({
              ...
              // Full payload for ViewProduct
              "images": images,
              "ingredients": item["ingredients"] ?? "",
              "howToUse": (item["howToUse"] is List)
                  ? (item["howToUse"] as List).map((e) => e.toString()).toList()
                  : <String>[],
            });
          }
