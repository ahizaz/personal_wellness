          ...
          Positioned(
            ...
            child: SafeArea(
              child: Container(
                ...
                // In SignInPassword.dart, inside the CustomButton for "Continue"
Obx(() => CustomButton(
  text: 'Continue',
  textStyle: TextStyle(
    fontSize: 17.sp,
  ...
  color: const Color(0xff172601),
  onTap: () async {
    final signInController = Get.find<SignInController>(); // Access SignInController
    final email = signInController.email.value;
    final password = controller.signInPasswordController.text;

    if (email.isNotEmpty && password.isNotEmpty) {
      bool success = await controller.login(email, password);
      if (success) {
        controller.signInPasswordController.clear();
        // Navigation is handled in controller.login() method
      }
    } else {
